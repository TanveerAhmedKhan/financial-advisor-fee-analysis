...
def create_unique_dataset(df):
    ...
    # Sort by filing date (descending) within each adviser-year group
    if 'Adviser_ID1' in unique_df.columns and 'Adviser_ID2' in unique_df.columns and 'Filing_Year' in unique_df.columns:
        # First sort by original order to maintain it as a secondary sort key
        unique_df = unique_df.sort_values('original_order')

        # Ensure Filing_Year is not NaN
        unique_df['Filing_Year'] = unique_df['Filing_Year'].fillna(-1)

        # Create a more robust uniqueness identifier using only Filing_Year
        # Since we've removed the redundant Year column
        unique_df['Effective_Year'] = unique_df['Filing_Year']

        # Convert to integer for proper grouping
        unique_df['Effective_Year'] = unique_df['Effective_Year'].fillna(-1).astype(int)

        # Then sort by adviser IDs, year, and filing date
        unique_df = unique_df.sort_values(
            ['Adviser_ID1', 'Adviser_ID2', 'Effective_Year', 'Filing_Date'],
            ascending=[True, True, True, False]
        )

        # Drop duplicates keeping the first occurrence (which will be the latest filing date)
        # Use Effective_Year instead of Filing_Year for deduplication
        unique_df = unique_df.drop_duplicates(subset=['Adviser_ID1', 'Adviser_ID2', 'Effective_Year'])

        # Remove the temporary column
        unique_df = unique_df.drop('Effective_Year', axis=1)
